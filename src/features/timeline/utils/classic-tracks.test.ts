...
describe('classic tracks', () => {
  ...
  it('renames generic tracks into classic names when assigning a kind', () => {
    const tracks = [makeTrack({ id: 'track-1', name: 'Track 1' }), makeTrack({ id: 'track-2', name: 'V1', kind: 'video', order: 1 })];
    expect(renameTrackForKind(tracks[0]!, tracks, 'audio')).toMatchObject({ kind: 'audio', name: 'A1' });
  });
  ...
  it('finds or creates adjacent classic track positions', () => {
    ...
    expect(findNearestTrackByKind({ tracks, targetTrack: tracks[0]!, kind: 'audio', direction: 'below' })?.id).toBe('a1');
    expect(getAdjacentTrackOrder(tracks, tracks[1]!, 'above')).toBe(0.5);
    expect(getNextClassicTrackName(tracks, 'audio')).toBe('A2');
    expect(createClassicTrack({ tracks, kind: 'audio', order: 3 })).toMatchObject({ name: 'A2', kind: 'audio' });
  });
});
