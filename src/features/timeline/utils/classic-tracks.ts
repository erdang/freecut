...
export function renameTrackForKind(track: TimelineTrack, tracks: TimelineTrack[], kind: TrackKind): TimelineTrack {
  const nextName = GENERIC_TRACK_NAME_REGEX.test(track.name)
    ? getNextClassicTrackName(tracks.filter((candidate) => candidate.id !== track.id), kind)
    : track.name;
