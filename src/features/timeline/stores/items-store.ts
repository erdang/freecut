...
export const useItemsStore = create<ItemsState & ItemsActions>()((set, get) => ({
  ...
  setItems: (items) =>
    set((state) => {
      ...
      const nextState = withItemIndexes(normalizedItems, state)
      return {
        ...nextState,
        replaceableCaptionClipIds: selectReplaceableCaptionClipIds(nextState),
      }
    }),
  setTracks: (tracks) =>
    set((state) => {
  ...
  _addItem: (item) =>
    set((state) => {
      ...
      const nextState = withItemIndexes(nextItems, state)
      return {
        ...nextState,
        replaceableCaptionClipIds: selectReplaceableCaptionClipIds(nextState),
      }
  ...
  _rippleDeleteItems: (ids) =>
    set((state) => {
      ...
      const nextState = withItemIndexes(newItems, state)
      return {
        ...nextState,
        replaceableCaptionClipIds: selectReplaceableCaptionClipIds(nextState),
      }
