...
function buildTransitionsByTrackId(
  ...
  for (const [trackId, trackTransitions] of Object.entries(grouped)) {
    ...
    next[trackId] =
      ...
      areTransitionArraysEqual(previousTrackTransitions, trackTransitions)
        ? previousTrackTransitions
        : trackTransitions
  }
...
export const useTransitionsStore = create<TransitionsState & TransitionsActions>()((set) => ({
  ...
  _removeTransitionsForItems: (itemIds) =>
    set((state) => {
