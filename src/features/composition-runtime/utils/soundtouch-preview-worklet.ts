...
const workletModuleUrl = new URL('../worklets/soundtouch-preview-processor.worklet.ts', import.meta.url);
...
export async function ensureSoundTouchPreviewWorkletLoaded(context: AudioContext): Promise<boolean> {
  ...
  const loadPromise = context.audioWorklet.addModule(workletModuleUrl.href)
    .then(() => true)
    .catch((error) => {
