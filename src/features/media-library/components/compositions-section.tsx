          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription asChild>
              <div className="space-y-3">
                <p>确定要删除 &ldquo;{deleteTarget?.name}&rdquo; 吗？此操作无法撤销。</p>
                {deleteImpact.totalReferenceCount > 0 && (
                  <div className="flex items-start gap-2 p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-md">
                    ...
                    <div className="text-sm text-yellow-600 dark:text-yellow-400">
                      <p className="font-medium">所有剩余实例都会被移除</p>
                      <p className="text-xs mt-1 text-yellow-600/80 dark:text-yellow-400/80">
                        时间线及嵌套复合片段中的 {deleteImpact.totalReferenceCount}{' '}
                        个复合片段实例也会一并删除。
                      </p>
                    </div>
                  </div>
  ...
  if (viewMode === 'grid') {
    return (
      <ContextMenu>
        ...
        <ContextMenuContent>
          <ContextMenuItem onClick={onEnter}>进入复合片段</ContextMenuItem>
