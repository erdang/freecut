...
const schedulePreviewWorkMock = vi.hoisted(() => vi.fn((task: () => void) => {
  scheduledTasksRef.current.push(task);
  return () => {
    scheduledTasksRef.current = scheduledTasksRef.current.filter((candidate) => candidate !== task);
  };
}));
...
describe('backgroundMediaWork', () => {
  ...
  it('waits for the active job before starting the next one', async () => {
    ...
    enqueueBackgroundMediaWork(() => new Promise<void>((resolve) => {
