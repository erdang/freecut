...
export function resolveRenderPumpTargetFrame({
  ...
  return state.previewFrame ?? ((forceFastScrubOverlay || isPausedInsideTransition)
    ? state.currentFrame
    : null);
}
...
export function resolveBackwardScrubFramePlan({
  ...
  const withinThrottle = (
    (nowMs - lastBackwardScrubRenderAt) < FAST_SCRUB_BACKWARD_RENDER_THROTTLE_MS
  );
  const jumpDistance = lastBackwardRequestedFrame === null
  ...
  return {
    ...
    throttleRequest: (
      withinThrottle
      && jumpDistance < FAST_SCRUB_BACKWARD_FORCE_JUMP_FRAMES
    ),
