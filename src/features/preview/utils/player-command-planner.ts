...
export function planPlaybackStateCommand(input: {
  ...
  if (isPlaying && !wasPlaying) {
    ...
    return {
      ...
      command: needsSeek
        ? { type: 'seek_and_play', targetFrame: currentFrame }
        : { type: 'play' },
    };
  }
...
export function planCurrentFrameSyncCommand(input: {
  ...
  if (transition.next.mode === 'playing') {
    const withinPlayingDriftWindow = playerFrame !== null && Math.abs(playerFrame - currentFrame) <= 2;
...
export function planPreviewFrameSyncCommand(input: {
  ...
  if (scrubDirection < 0) {
    ...
    const jumpDistance = backwardScrubState.lastSeekFrame === null
