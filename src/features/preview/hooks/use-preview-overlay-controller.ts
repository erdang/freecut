...
export function usePreviewOverlayController({
  ...
  const maybeCompleteFastScrubHandoff = useCallback(
    (resolvedFrame?: number | null) => {
      ...
      if (playerFrame === null) {
        ...
        playerFrame = Number.isFinite(currentFrame) ? Math.round(currentFrame as number) : null
      }
  ...
  const beginFastScrubHandoff = useCallback(
    (targetFrame: number) => {
