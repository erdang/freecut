...
export function usePreviewTransitionModel({
  ...
  const getTransitionWindowByStartFrame = useCallback(
  ...
  const getTransitionCooldownForWindow = useCallback(
  ...
  const getTransitionWindowForFrame = useCallback(
    (frame: number) => {
      return (
        playbackTransitionWindows.find(
          (window) =>
            frame >= window.startFrame &&
            frame < window.endFrame + getTransitionCooldownForWindow(window),
        ) ?? null
      )
    },
  ...
  const getActiveTransitionWindowForFrame = useCallback(
    (frame: number) => {
      return (
        playbackTransitionWindows.find(
  ...
  const shouldPreserveHighFidelityBackwardPreview = useCallback(
    (frame: number | null) => {
...
export function buildPreviewTransitionData({
  ...
  const pausedTransitionPrearmFrames = Math.max(
    playbackTransitionLookaheadFrames,
    Math.round(fps * 3),
  )
  const playingComplexTransitionPrearmFrames = Math.max(
    playbackTransitionLookaheadFrames,
    Math.round(fps * 1.5),
  )
