...
export function LocalModelCacheControl({ className }: LocalModelCacheControlProps) {
  ...
  const handleClear = useCallback(
    async (summary: LocalModelCacheSummary) => {
      ...
      try {
        ...
        toast.success(deleted ? `已清理 ${summary.label}` : `${summary.label} 本身为空`)
      } catch (error) {
  ...
  return (
    <div className={cn('space-y-3', className)}>
      ...
      {!inspectionSupported && (
        <p className="text-xs text-muted-foreground">当前环境不支持缓存探测。</p>
      )}
