  ...
  return (
    ...
    >
      {palette.map((swatch, i) => {
        ...
        const isHighlighted =
          highlight && Boolean(palette[i]) && paletteMatchesFamily(palette[i]!, highlight)
...
function paletteMatchesFamily(swatch: Swatch, family: string): boolean {
  ...
  switch (family) {
    case 'red':
      return hueDeg < 20 || hueDeg >= 345
    case 'orange':
      return hueDeg >= 20 && hueDeg < 50
    case 'yellow':
      return hueDeg >= 50 && hueDeg < 80
    case 'green':
      return hueDeg >= 95 && hueDeg < 165
    case 'teal':
      return hueDeg >= 165 && hueDeg < 200
    case 'blue':
      return hueDeg >= 200 && hueDeg < 255
    case 'purple':
      return hueDeg >= 255 && hueDeg < 300
    case 'pink':
      return hueDeg >= 300 && hueDeg < 345
    case 'brown':
      return hueDeg >= 20 && hueDeg < 60 && l < 50
    default:
      return false
  }
}
