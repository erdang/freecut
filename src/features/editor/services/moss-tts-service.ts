...
function mergeChunkChannels(audioChunks: SerializedAudioChunk[]): { channels: Float32Array[]; sampleRate: number } {
  let sampleRate = 0;
  let channelCount = 0;
...
function createFloat32WavBlob(channels: Float32Array[], sampleRate: number): Blob {
  ...
  for (let frameIndex = 0; frameIndex < frameCount; frameIndex += 1) {
    for (let channelIndex = 0; channelIndex < channelCount; channelIndex += 1) {
      interleaved[frameIndex * channelCount + channelIndex] = channels[channelIndex]?.[frameIndex] ?? 0;
    }
  }
...
export function getMossTtsVoiceOption(voice: MossTtsVoice): { value: MossTtsVoice; label: string } {
  return MOSS_TTS_VOICE_OPTIONS.find((option) => option.value === voice) ?? {
    value: voice,
    label: voice,
  };
}
...
class MossTtsService {
  ...
  private upsertRuntime(state: 'loading' | 'running' | 'ready' | 'error', errorMessage?: string): void {
  ...
  private readonly handleWorkerMessage = (event: MessageEvent<ReadyInfo | ProgressInfo | ResponseInfo>) => {
  ...
  async generateSpeechFile({
    ...
    return this.withGenerationLock(async () => {
      ...
      try {
        ...
        const response = await this.requestWorker('synthesize', {
          text: trimmedText,
          voiceName: voice,
        }, onProgress);
