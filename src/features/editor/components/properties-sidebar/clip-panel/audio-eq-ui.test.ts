...
describe('audio-eq-ui control ranges', () => {
  ...
  it('clamps to the selected Davinci-style range when switching control bands', () => {
    expect(clampFrequencyToAudioEqControlRange(120, 'H')).toBe(getAudioEqControlRangeById('H').minFrequencyHz);
    expect(clampFrequencyToAudioEqControlRange(18000, 'ML')).toBe(getAudioEqControlRangeById('ML').maxFrequencyHz);
  });
});
