...
export function calculateMediaCropLayout(
  ...
  const outerExpansion: CropInsets = rawSoftnessPixels > 0
    ? {
  ...
  const featherInput: CropInsets = {
    ...
    right: cropPixels.right > 0 ? (rawSoftnessPixels > 0 ? outerExpansion.right : softnessPixels) : 0,
    top: cropPixels.top > 0 ? (rawSoftnessPixels > 0 ? outerExpansion.top : softnessPixels) : 0,
    bottom: cropPixels.bottom > 0 ? (rawSoftnessPixels > 0 ? outerExpansion.bottom : softnessPixels) : 0,
  };
